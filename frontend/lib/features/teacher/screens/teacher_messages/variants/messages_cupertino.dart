...
class MessagesCupertino extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    const messages = [
      _MessageData(
          'John Smith', 'Question about homework.', '10:30 AM', true, 'JS'),
      _MessageData('Sarah Johnson', 'Project feedback received.', 'Yesterday',
          false, 'SJ'),
      _MessageData(
          'Michael Brown', 'Next class meeting?', 'Yesterday', true, 'MB'),
      _MessageData(
          'Emily Davis', 'Assignment submitted.', 'Monday', false, 'ED'),
    ];
  ...
  Widget _buildIosHeader() {
    return Column(
      ...
      children: [
        ...
        Text('Communication Hub'.toUpperCase(),
              ...
              return Column(
                children: [
                  ...
                  if (index < messages.length - 1)
                    const Divider(
                        height: 1, indent: 64, color: Color(0xFFF2F2F7)),
                ],
              );
            }).toList(),
          ),
...
class _MessageData {
  ...
  final bool isUnread;
  const _MessageData(
      this.name, this.message, this.time, this.isUnread, this.avatar);
}
