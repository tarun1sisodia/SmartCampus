...
export default function OrganisationDetailPage({ params }: Props) {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => getOrganisationById(params.id)
  });

  const mutation = useMutation({
    mutationFn: (payload: OrganisationFormValues) => updateOrganisation(params.id, payload),
    onSuccess: async () => {
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading || !data ? (
        <div className="card text-sm text-textSecondary">Loading organisation details...</div>
      ) : (
        <OrganisationForm
