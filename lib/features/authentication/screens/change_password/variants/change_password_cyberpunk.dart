...
class ChangePasswordCyberpunk extends StatelessWidget {
  ...
  Widget _buildCyberHeader(Color cyan, Color magenta) {
    return Column(
      children: [
        ...
        Text('KEY_ROTATION', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 32, color: cyan, letterSpacing: 4, fontFamily: 'Courier')),
        const SizedBox(height: 12),
        Text('INITIATING_IDENTITY_RE_ESTABLISHMENT...', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 10, color: magenta, letterSpacing: 2, fontFamily: 'Courier')),
      ],
    );
  }

  Widget _buildCyberForm(ChangePasswordController controller, Color color) {
    return Form(
      key: controller.changePasswordFormKey,
      child: Column(
        children: [
          _buildCyberField(controller.newPassword, 'NEW_IDENTITY_KEY', color, controller),
          const SizedBox(height: 16),
          _buildCyberField(controller.confirmPassword, 'CONFIRM_IDENTITY_KEY', color, controller),
          const SizedBox(height: 56),
          GestureDetector(
  ...
  Widget _buildCyberField(TextEditingController textController, String label, Color color, ChangePasswordController controller) {
