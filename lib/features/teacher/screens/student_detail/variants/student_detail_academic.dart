...
class StudentDetailAcademic extends StatelessWidget {
      ...
      children: [
        GestureDetector(
          ...
          child: Container(
             ...
             decoration: BoxDecoration(color: Colors.white, border: Border.all(color: accent.withOpacity(0.3))),
             child: Padding(
                padding: const EdgeInsets.all(4),
                child: hasImg 
                   ? CachedNetworkImage(imageUrl: ApiConstants.optimizeImageUrl(cur!.imageUrl!, width: 200, height: 200), fit: BoxFit.cover)
                   : Icon(Iconsax.user, color: accent.withOpacity(0.3), size: 40),
             ),
          ),
        ),
        const SizedBox(height: 24),
        Text(cur?.name ?? student.name, style: TextStyle(fontWeight: FontWeight.w800, fontSize: 20, color: ink, fontFamily: 'Serif')),
        Text('Roll Number: ${cur?.rollNumber ?? student.rollNumber}', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: ink.withOpacity(0.5), fontFamily: 'Serif')),
      ],
    );
  }
  ...
  Widget _stNode(String l, String v, Color ink) {
    return Column(children: [Text(v, style: TextStyle(fontWeight: FontWeight.w800, fontSize: 24, color: ink, fontFamily: 'Serif')), Text(l, style: TextStyle(fontWeight: FontWeight.w600, fontSize: 10, color: ink.withOpacity(0.4), fontFamily: 'Serif'))]);
  }
  ...
  Widget _buildScholarHistory(BuildContext context, Color accent, Color ink) {
    ...
    return Column(
       ...
       children: [
          Text('Enrollment History'.toUpperCase(), style: TextStyle(fontWeight: FontWeight.w900, fontSize: 10, letterSpacing: 1.5, color: accent.withOpacity(0.4), fontFamily: 'Serif')),
          ...
          ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return Container(
                ...
                decoration: BoxDecoration(color: Colors.white, border: Border.all(color: accent.withOpacity(0.05))),
                child: ListTile(
                   ...
                   subtitle: Text('${rec['session'].startTime} - ${rec['session'].endTime}', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 11, color: ink.withOpacity(0.4), fontFamily: 'Serif')),
  ...
  void _updateRec(BuildContext context, String sid, String cur, String? rem, Color accent, Color ink) {
     ...
     Get.dialog(AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
           ...
           const SizedBox(height: 24),
           TextField(controller: t, style: TextStyle(fontFamily: 'Serif', fontSize: 13), decoration: InputDecoration(labelText: 'Observation Remarks', labelStyle: TextStyle(fontSize: 10, fontFamily: 'Serif'), border: UnderlineInputBorder(borderSide: BorderSide(color: accent.withOpacity(0.2))))),
        ]),
        actions: [
  ...
  Widget _btn(String l, Color c, bool s, VoidCallback t, Color accent) {
     return InkWell(onTap: t, child: Container(width: 44, height: 44, decoration: BoxDecoration(color: s ? accent.withOpacity(0.1) : Colors.transparent, border: Border.all(color: s ? accent : Colors.black12)), child: Center(child: Text(l, style: TextStyle(color: s ? accent : Colors.black26, fontWeight: FontWeight.w900)))));
  }
  ...
  Widget _pTile(String l, IconData i, Color c, VoidCallback t) {
     return InkWell(onTap: t, child: Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(16), decoration: BoxDecoration(border: Border.all(color: c.withOpacity(0.1)), color: const Color(0xFFFAF7F0)), child: Row(children: [Icon(i, color: c, size: 20), const SizedBox(width: 16), Text(l, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 12, fontFamily: 'Serif'))])));
  }
}
