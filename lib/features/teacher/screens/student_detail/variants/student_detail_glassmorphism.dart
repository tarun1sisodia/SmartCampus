...
class StudentDetailGlassmorphism extends StatelessWidget {
          ...
          GestureDetector(
             ...
             child: Stack(
                children: [
                   ...
                   Positioned(right: 0, bottom: 0, child: Container(padding: const EdgeInsets.all(4), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.2), shape: BoxShape.circle), child: const Icon(Iconsax.camera, color: Colors.white, size: 14))),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('Roll ID: ${currentStudent?.rollNumber ?? student.rollNumber}', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 11, color: Colors.white.withValues(alpha: 0.7))),
              ],
            ),
          ),
        ],
          ...
          Row(
            ...
            children: [
               ...
               CircularPercentIndicator(
                  ...
                  progressColor: Colors.white, backgroundColor: Colors.white.withValues(alpha: 0.1), circularStrokeCap: CircularStrokeCap.round,
               ),
            ],
          ),
  ...
  Widget _stItem(String l, String v, Color c) {
    return Column(children: [Text(v, style: TextStyle(fontWeight: FontWeight.w900, fontSize: 20, color: c)), Text(l, style: TextStyle(fontWeight: FontWeight.w800, fontSize: 9, color: Colors.white.withValues(alpha: 0.5)))]);
  }
  ...
  Widget _buildGlassHistory(BuildContext context) {
    ...
    return Column(
       crossAxisAlignment: CrossAxisAlignment.start,
       children: [
          Padding(padding: const EdgeInsets.only(left: 8), child: Text('ACTIVITY_LEDGER', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 10, letterSpacing: 1, color: Colors.white.withValues(alpha: 0.5)))),
              ...
              return Container(
                ...
                child: _GlassContainer(
                   ...
                   child: ListTile(
                      ...
                      leading: Container(width: 40, height: 40, decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.1), shape: BoxShape.circle), child: Center(child: Icon(_getIcon(status), color: Colors.white, size: 18))),
                      ...
                      subtitle: Text('${session.startTime} > ${session.endTime}', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 10, color: Colors.white.withValues(alpha: 0.5))),
                      trailing: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8)), child: Text(status.toUpperCase(), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 8))),
                   ),
                ),
              );
            },
  ...
  void _updateRec(BuildContext context, String sid, String cur, String? rem) {
     ...
     Get.bottomSheet(_GlassContainer(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
           ...
           const SizedBox(height: 24),
           TextField(controller: rc, style: const TextStyle(color: Colors.white), decoration: InputDecoration(hintText: 'Notes...', hintStyle: TextStyle(color: Colors.white.withValues(alpha: 0.3)), filled: true, fillColor: Colors.white.withValues(alpha: 0.05), border: OutlineInputBorder(borderSide: BorderSide.none))),
           const SizedBox(height: 32),
    ...
    return ClipRRect(
      child: BackdropFilter(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.1), border: Border.all(color: Colors.white.withValues(alpha: 0.2)), borderRadius: BorderRadius.circular(24)),
          child: child,
        ),
      ),
    );
