import 'package:SmartCampus/models/subject_model.dart';
...
class SubjectService {
  ...
  Future<List<SubjectModel>> getAllSubjects() async {
    try {
      final response = await supabase.from('subjects').select().order('name');
  ...
  Future<SubjectModel> createSubject(String name, String code) async {
    try {
      ...
      final response =
          await supabase.from('subjects').insert(data).select().single();
